...
contract DeployDodoV2Oracle is Script {
    function run() external {
        ...
        vm.startBroadcast();
        DodoV2Oracle oracle = new DodoV2Oracle{salt: salt}(IDVMFactory(factory));
        oc.addOracle(oracle, OffchainOracle.OracleType(oracleType));
        vm.stopBroadcast();

        console.log("OffchainOracle:", address(oc));
        console.log("DODO V2 Factory:", factory);
        console.log("Oracle type:", oracleType);
        console.log("DodoV2Oracle deployed at:", address(oracle));
    }
}
