...
contract OffchainOracle is Ownable {
    ...
    function oracles() public view returns (IOracle[] memory allOracles, OracleType[] memory oracleTypes) {
        unchecked {
            IOracle[] memory oraclesBuffer =
                new IOracle[](_wethOracles._inner._values.length + _ethOracles._inner._values.length);
            ...
            for (uint256 i = 0; i < _ethOracles._inner._values.length; i++) {
                ...
                for (uint256 j = 0; j < oraclesBuffer.length; j++) {
    ...
    function getRateWithCustomConnectors(
        ...
        (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) = getRatesAndWeightsWithCustomConnectors(
            srcToken, dstToken, useWrappers, customConnectors, thresholdFilter
        );
        if (wrappedPrice > 0) return wrappedPrice;
     ...
     * @param customConnectors An array of custom connectors to use
     * @param thresholdFilter The threshold percentage (from 0 to 100) used to filter out rates below the threshold
     * @return wrappedPrice The wrapped rate
     * @return ratesAndWeights {OraclePrices.Data} object containing the rates and weights from different oracles if wrappedPrice is 0
     */
    function getRatesAndWeightsWithCustomConnectors(
        IERC20 srcToken,
        IERC20 dstToken,
        bool useWrappers,
        IERC20[] memory customConnectors,
        uint256 thresholdFilter
    ) public view returns (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) {
        ...
        uint256 maxArrLength = wrappedSrcTokens.length * wrappedDstTokens.length
            * (allConnectors[0].length + allConnectors[1].length) * allOracles.length;
     ...
     * @notice The same as `getRate` but checks against `ETH` and `WETH` only
     */
    function getRateToEthWithThreshold(IERC20 srcToken, bool useSrcWrappers, uint256 thresholdFilter)
        external
        view
        returns (uint256 weightedRate)
    {
    ...
    function getRateToEthWithCustomConnectors(
        ...
        (uint256 wrappedPrice, OraclePrices.Data memory ratesAndWeights) = getRatesAndWeightsToEthWithCustomConnectors(
            srcToken, useSrcWrappers, customConnectors, thresholdFilter
        );
        if (wrappedPrice > 0) return wrappedPrice;
        (weightedRate,) = ratesAndWeights.getRateAndWeightWithSafeMath(thresholdFilter);
    }
     ...
     *    Usage of the dex oracle on chain is highly discouraged!
     *    getRate function can be easily manipulated inside transaction!
     * @notice The same as `getRatesAndWeightsWithCustomConnectors` but checks against `ETH` and `WETH` only
     */
    function getRatesAndWeightsToEthWithCustomConnectors(
        IERC20 srcToken,
        bool useSrcWrappers,
        IERC20[] memory customConnectors,
        uint256 thresholdFilter
        ...
        uint256 maxArrLength = wrappedSrcTokens.length * wrappedDstTokens.length
            * (allConnectors[0].length + allConnectors[1].length)
            * (wrappedOracles[0].length + wrappedOracles[1].length);
        ratesAndWeights = OraclePrices.init(maxArrLength);
        unchecked {
            for (uint256 k1 = 0; k1 < wrappedSrcTokens.length; k1++) {
                for (uint256 k2 = 0; k2 < wrappedDstTokens.length; k2++) {
    ...
    function _getWrappedTokens(IERC20 token, bool useWrappers)
        internal
        view
    ...
    function _getAllConnectors(IERC20[] memory customConnectors)
        internal
        view
        returns (IERC20[][2] memory allConnectors)
    {
    ...
    function _getRateImpl(GetRateImplParams memory p)
        private
        view
        returns (OraclePrices.OraclePrice memory oraclePrice)
    {
        try p.oracle.getRate(p.srcToken, p.dstToken, p.connector, p.thresholdFilter) returns (
